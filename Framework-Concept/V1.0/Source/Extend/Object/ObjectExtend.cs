...
namespace FS.Extend
{
    ...
    public static class ObjectExtend
    {
        ...
        public static T ConvertType<T>(this object objValue, T defValue = default(T))
        {
            ...
            // 相同类型，则直接返回原型
            if (Type.GetTypeCode(defType) == Type.GetTypeCode(objType)) { return (T)objValue; }

            var val = objValue.ConvertType(defType);
        ...
        public static object ConvertType(this object objValue, Type defType)
        {
            if (objValue == null) { return null; }

            // 对   Nullable<> 类型处理
            if (defType.IsGenericType && defType.GetGenericTypeDefinition() == typeof(Nullable<>)) { return objValue.ConvertType(Nullable.GetUnderlyingType(defType)); }
            // 对   List 类型处理
            ...
            return ConvertType(objValue, objValue.GetType(), defType);
        }
