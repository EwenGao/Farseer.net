...
namespace FS.Utils.Web
{
    ...
    public abstract class Req
    {
        ...
        {
            string domain = HttpContext.Current == null ? string.Empty : HttpContext.Current.Request.Url.Authority;
        ...
        public static TInfo Fill<TInfo>(NameValueCollection request, Action<string, string> tip = null, string url = "", string prefix = "hl") where TInfo : BaseInfo, new()
        {
            if (tip == null)
            {
        ...
        public static TInfo Fill<TInfo>(NameValueCollection request, Action<Dictionary<string, List<string>>> tip,
                                        string prefix = "hl") where TInfo : BaseInfo, new()
        {
            Dictionary<string, List<string>> dicError;
            var info = Fill<TInfo>(request, out dicError, prefix);
            if (dicError.Count > 0)
        ...
        public static TInfo Fill<TInfo>(NameValueCollection request, out Dictionary<string, List<string>> dicError, string prefix = "hl") where TInfo : BaseInfo, new()
        {
            dicError = new Dictionary<string, List<string>>();
            var t = new TInfo();
            foreach (var kic in ModelCache.GetInfo(typeof(TInfo)).ModelList)
            {
                ...
                if (request[reqName].Trim().Length > 0 && obj == null)
                {
                    ...
                    {
                        case "DateTime":
                            lstError.Add("[" + kic.Value.Display.Name + "] 必需为时间格式。如：2011-09-01");
                            break;
                        case "Boolean":
                            lstError.Add("[" + kic.Value.Display.Name + "] 必需为布尔格式。如：false、true");
                            break;
                        case "UInt64":
                            lstError.Add("[" + kic.Value.Display.Name + "] 必需为整形格式。");
                            break;
                        case "Int64":
                            lstError.Add("[" + kic.Value.Display.Name + "] 必需为整形格式。");
                            break;
                        case "Int32":
                            lstError.Add("[" + kic.Value.Display.Name + "] 必需为整形格式2。");
                            break;
                        case "Int16":
                            lstError.Add("[" + kic.Value.Display.Name + "] 必需为整形格式。");
                            break;
                        case "Decimal":
                            lstError.Add("[" + kic.Value.Display.Name + "] 必需为数字格式。");
                            break;
                        case "Byte":
                            lstError.Add("[" + kic.Value.Display.Name + "] 必需为字节格式。");
                            break;
                        case "Long":
                            lstError.Add("[" + kic.Value.Display.Name + "] 必需为整形格式。");
                            break;
