...
namespace FS.Utils.Common
{
    ...
    public abstract class Net
    {
        ...
        /// <param name="wc"></param>
        public static int DownloadFile(string url, string savePath, WebClient wc)
        {
        ...
        public static string DownloadData(string url, Encoding encoding, ref CookieContainer cookie)
        {
            url = Url.ConvertPath(url);
            if (url[0] == '/') { url = Req.GetDomain() + url; }
            if (url.IsNullOrEmpty()) { return string.Empty; }
