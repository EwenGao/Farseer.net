...
namespace FS.Core.Bean
{
    ...
    internal class SQLiteBuilder<TInfo> : DbBuilder<TInfo> where TInfo : ModelInfo, new()
    {
        /// <summary>
        ///     SqLite数据库Sql生成
        /// </summary>
        /// <param name="tableName">表名称</param>
        internal SQLiteBuilder(string tableName = "") : base(tableName)
        {
        ...
        {
            return string.Format("select {0} from {1} {2} {3} LIMIT 0,1;", GetFields(), TableName, WhereString, SortString);
        }

        public override string GetValue()
        {
            return string.Format("SELECT {0} FROM {1} {2} {3} LIMIT 0,1;", GetFields(), TableName, WhereString,
                                 SortString);
        }
