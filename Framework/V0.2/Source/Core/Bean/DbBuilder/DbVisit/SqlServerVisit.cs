...
namespace FS.Core.Bean
{
    ...
    internal class SqlServerVisit<TInfo> : DbVisit<TInfo> where TInfo : ModelInfo, new()
    {
        internal SqlServerVisit()
        {
        ...
        protected override Expression VisitMethodCall(MethodCallExpression m)
        {
            ...
            switch (m.Method.Name.ToUpper())
            {
                ...
                case "STARTSWITH":
                    {
                        SqlList.Push(String.Format("CHARINDEX({1},{0}) = 1", fieldName, paramName));
                        break;
                    }
                case "ENDSWITH":
                    {
